...
import { ReachPolygon, OriginMarker, ServicePin, HatchArea, servicesInPolygon } from '@/components/MapOverlays';
import { usePrefersReducedMotion, useCountUp, useStaggeredReveal } from '@/hooks/useAnimations';
import { generateReachPolygon, polygonArea, mapAreaToKm2, pointInPolygon, type MapPoint } from '@/utils/geometry';
import { SERVICES, CITY_CENTER, CATEGORY_META, CATEGORY_ORDER, type ServiceCategory } from '@/data/mockData';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        <CityMap showTransit showRoads />
        <svg viewBox="0 0 1000 700" className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="xMidYMid slice">
          <ReachPolygon points={eveningPoly} color="#6366f1" fillOpacity={0.15} animate={!reduced} />
        </svg>
      </div>
      {/* Morning (clipped) */}
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${dividerPos}%` }}>
        <div style={{ width: `${100 / (dividerPos / 100)}%`, height: '100%' }}>
          <CityMap showTransit showRoads />
          <svg viewBox="0 0 1000 700" className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="xMidYMid slice">
            <ReachPolygon points={morningPoly} color="#f59e0b" fillOpacity={0.15} animate={!reduced} />
          </svg>
        </div>
      </div>
      {/* Divider */}
