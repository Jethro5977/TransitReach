...
import { ReachPolygon, OriginMarker, ScenarioRouteOverlay, HatchArea, servicesInPolygon } from '@/components/MapOverlays';
...
import { SERVICES, SCENARIO_ROUTES, TRANSIT_LINES, CITY_CENTER, type ScenarioRoute } from '@/data/mockData';
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-lg" style={{ aspectRatio: '10/7', minHeight: '400px' }}>
            ...
            <svg viewBox="0 0 1000 700" className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="xMidYMid slice">
              ...
              {(tab === 'baseline' || tab === 'difference') && (
                <ReachPolygon points={baselinePoly} color="#94a3b8" fillOpacity={tab === 'difference' ? 0.08 : 0.15} animate={!reduced} />
              )}
              {/* Scenario polygon */}
              {(tab === 'scenario' || tab === 'difference') && showNewPolygon && (
                <ReachPolygon points={scenarioPoly} color="#14b8a6" fillOpacity={0.15} animate={!reduced} />
              )}
              {/* Difference hatches */}
